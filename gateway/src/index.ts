...
// ---------------------------------------------------------------------------
function isPassthroughPath(url: string): boolean {
  const path = url.split('?')[0];
  return (
    path.startsWith('/api/') ||
    path === '/api' ||
...
const healthCache: Record<string, { healthy: boolean; ts: number }> = {};
...
function invalidateHealth(targetUrl: string) {
...
const outlinePassthroughProxy = httpProxy.createProxyServer({
  target: OUTLINE_URL,
  ws: true,
  changeOrigin: true,
});
...
const outlineHtmlProxy = httpProxy.createProxyServer({
  target: OUTLINE_URL,
...
// ---------------------------------------------------------------------------
function setForwardedHeaders(proxyReq: http.ClientRequest, req: IncomingMessage) {
...
// ---------------------------------------------------------------------------
outlinePassthroughProxy.on('proxyReq', (proxyReq: http.ClientRequest, req: IncomingMessage) => {
  setForwardedHeaders(proxyReq, req);
  log('debug', `[passthrough] proxying ${req.method} ${req.url}`);
});

outlinePassthroughProxy.on('error', (err: Error, req: IncomingMessage, res: ServerResponse | import('net').Socket) => {
...
// ---------------------------------------------------------------------------
outlineHtmlProxy.on('proxyReq', (proxyReq: http.ClientRequest, req: IncomingMessage) => {
  setForwardedHeaders(proxyReq, req);
...
outlineHtmlProxy.on('proxyRes', async (proxyRes: IncomingMessage, req: IncomingMessage, res: ServerResponse) => {
...
outlineHtmlProxy.on('error', (err: Error, req: IncomingMessage, res: ServerResponse | import('net').Socket) => {
  invalidateHealth(OUTLINE_URL);
