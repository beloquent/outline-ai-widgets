import type { WidgetContext, WidgetDefinition } from '../../sdk/types';
...
class GraphViewWidget {
  ...
  private collectionColorMap: Map<string, string> = new Map();
  private currentDocumentId: string | null = null;
  private isLocalMode = false;
  private depth = 2;
  private keyboardListenerAttached = false;

  onMount(container: HTMLElement, context: WidgetContext): void {
  ...
  private render(): void {
    ...
    const btn = this.container.querySelector('#graph-view-btn') as HTMLButtonElement;
  ...
  private createOverlay(): void {
    ...
    this.overlay.addEventListener('click', (e) => {
      if (e.target === this.overlay) this.hideGraph();
    });
    ...
    this.overlay.querySelector('#graph-mode-local')?.addEventListener('click', () => {
      if (this.isLocalMode || !this.currentDocumentId) return;
      this.isLocalMode = true;
      this.hideGraph();
      this.showGraph();
    });

    const depthInput = this.overlay.querySelector('#graph-depth') as HTMLInputElement;
    if (depthInput) {
      depthInput.addEventListener('input', (e) => {
        this.depth = parseInt((e.target as HTMLInputElement).value);
        const depthValue = this.overlay?.querySelector('#graph-depth-value');
        if (depthValue) depthValue.textContent = String(this.depth);
        this.loadGraphData();
      });
    }
  ...
  private async loadCollections(): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        const select = this.overlay?.querySelector('#graph-collection-filter') as HTMLSelectElement;
        if (select) {
  ...
  private async loadGraphData(): Promise<void> {
    ...
    if (loading) loading.textContent = 'Loading graph...';
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (loading) loading.textContent = error.message || 'Failed to load graph';
        return;
      }
      ...
      const collectionFilter = (this.overlay?.querySelector('#graph-collection-filter') as HTMLSelectElement)?.value;
      const showOrphans = (this.overlay?.querySelector('#graph-show-orphans') as HTMLInputElement)?.checked ?? true;
  ...
  private renderGraph(nodes: GraphNode[], edges: GraphEdge[], centerId?: string): void {
    ...
    const loading = this.overlay?.querySelector('#graph-loading') as HTMLElement;
    ...
    try {
      ...
      if (loading) loading.textContent = 'Error rendering graph';
    }
  }
