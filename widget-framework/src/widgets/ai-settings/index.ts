...
interface SettingsConfig {
  ...
  availableModels: Array<{ id: string; name: string; maxTokens: number }>;
}
...
type TabId = 'general' | 'documentation' | 'workflow' | 'sop' | 'kbChat';
...
class AISettingsWidget {
  ...
  private editedPrompts: Partial<ModePrompts> = {};
  private eventUnsubscribe: (() => void) | null = null;
  private isReindexing = false;
  private reindexStatus: { jobId?: string; documentsIndexed?: number; documentsQueued?: number; status?: string } | null = null;
  ...
  async mount(container: HTMLElement, context: WidgetContext): Promise<void> {
    ...
    // Subscribe to open event from AI Copilot
    ...
    // Expose show function on window for direct access (fallback)
    (window as any).__aiSettingsWidget = {
      show: () => this.show(),
      hide: () => this.hide(),
    };
    console.log('[AI Settings] Exposed __aiSettingsWidget on window');
    
    // Render immediately (hidden state)
    this.render();
  }
  ...
  unmount(): void {
    ...
    if ((window as any).__aiSettingsWidget) {
      delete (window as any).__aiSettingsWidget;
    }
    if (this.container) {
  ...
  private async saveConfig(): Promise<void> {
    ...
    const form = this.container.querySelector('#ai-settings-form') as HTMLFormElement;
    if (!form) return;

    const formData = new FormData(form);
    const openaiKey = formData.get('openaiApiKey') as string;
    const outlineKey = formData.get('outlineApiKey') as string;
    const model = (formData.get('model') as string) || this.config?.features.copilot.model || 'gpt-4o-mini';
  ...
  private async startReindex(): Promise<void> {
  ...
  private async pollReindexStatus(jobId: string): Promise<void> {
    const maxAttempts = 120; // 2 minutes max
    let attempts = 0;
    let consecutiveErrors = 0;

    const poll = async () => {
      // Check if modal is still visible and reindexing is active
  ...
  private updatePrompt(mode: keyof ModePrompts, value: string): void {
  ...
  private render(): void {
    ...
    // If not visible, render empty container
    if (!this.isVisible) {
    ...
    const modelOptions = this.config?.availableModels
      .map(m => `<option value="${m.id}" ${this.config?.features.copilot.model === m.id ? 'selected' : ''}>${m.name}</option>`)
      .join('') || '';
  ...
  private renderTabContent(theme: OutlineTheme, modelOptions: string): string {
    ...
    const modeKey = this.activeTab as keyof ModePrompts;
  ...
  private attachEventListeners(): void {
    const overlay = this.container?.querySelector('#ai-settings-overlay') as HTMLElement;
    const modal = this.container?.querySelector('.ai-settings-modal') as HTMLElement;
    ...
    const temperatureRange = this.container?.querySelector('#temperature-range') as HTMLInputElement;
    ...
    modeTextareas?.forEach(textarea => {
      textarea.addEventListener('input', (e) => {
        ...
        const mode = target.getAttribute('data-mode') as keyof ModePrompts;
        if (mode) {
